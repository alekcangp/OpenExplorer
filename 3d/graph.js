...
{"id": "https://seed1.switcheo.network:10331", "ip": "52.76.49.136", "color": "red", "val":20 },
{"id": "https://seed2.switcheo.network:10331", "ip": "52.76.49.136", "color": "red", "val":20 },
{"id": "https://seed3.switcheo.network:10331", "ip": "52.76.75.196", "color": "red", "val":20 },//!!
{"id": "https://seed4.switcheo.network:10331", "ip": "52.76.49.136", "color": "red", "val":20 },//!!
{"id": "https://seed5.switcheo.network:10331", "ip": "52.76.49.136", "color": "red", "val":20 },
...
{"id": "http://neo-node.com:10332", "ip": "192.64.119.185", "color": "red"},

//{"id": "http://neo-mn4.westeurope.cloudapp.azure.com:10332", "ip": "137.117.155.93", "color": "red"},

];

var bestbl, ind, counter2, bestago, memp, d, cc = 0, peersx = [], blocks = [], mems = [], memsago = [], ips = [], dubl=[], linksx = [], pcou = [], diffx = [], bb, pp, memrms = 0, repeatt = 1,
txt = "SWITCH TO TABLE VIEW";

for (var i = 0; i < nodesg.length; ++i) { // get ip of node
  let u = nodesg[i].id
  if ( isNaN(u.substr(8,1)) ) {
  reverce(u, i)
  } else { 
    ips[i] = nodesg[i].ip 
  } 
...
function reverce(u, i) {
  let ur = u.substr( u.indexOf(':')+3, u.indexOf(':',6)-u.indexOf(':',0)-3 );
  axios.get("http://api.ipapi.com/api/" + ur + "?access_key=6fe32fde04e86acd8117cec98d991a5e&fields=ip").then(function (result) { //api2 6fe32fde04e86acd8117cec98d991a5e; api1 5fb048044f352d455f87eb02b0f5d830
    ips[i] = result.data.ip; 
    ( isNaN(ips[i].substr(0,1)) || ips[i].indexOf(':') != -1 ) ? ips[i] = nodesg[i].ip : {};
  }).catch(function() { ips[i] = nodesg[i].ip })
}
...
// calculate all parameters
 ...
 // get links and color
    ...
    for (var k of peersx[j]) { ++pc;
      for (var t = 0; t < ips.length; ++t) {
       if (ips[t] == k.address) { 
        if (blocks[t] == undefined) {continue} //detect unreachable nodes
        ++pp;
        linksx.push({
        source : nodesg[j].id,
        target : nodesg[t].id
...
*/
      (repeatt == 1) ? timerun() : {}; //run funct if error
// run, reset timer
    if (bestbl != bestago) {  
      if (bestago != undefined)  {counter2 = -1} else  {getl(); setInterval(runbest, 10000)} // run timerun only one time
      bestago = bestbl;
    } 
   ...
   // var summ = dubl.reduce(function(sum, current) {
    ...
    for (var j = 0; j < nodesg.length; ++j) { // calculate avg  mem
          if (bestbl == blocks[j] && mems[j] != undefined) {//++d;
            mempp.push(mems[j]**2);
            } 
         }
         mempp.sort(function (a, b) { return a - b }); 
         memp = 0 ; d = 0.001;
         for (var m = (mempp.length)/3; m < (mempp.length)*2/3; m++) { 
           ++d;
           m = m.toFixed(0);
           memp += mempp[m]; 
         }
...
/*
    for (var i = 0; i < nodesg.length; ++i) {
         if (bestbl == blocks[i] && mems[i] != undefined && dubl[i] == 1 && Math.abs(memrms - mems[i]) < 250 ) {++cc; // detect only correct nodes
          if (cc == 1) {memp = 0; d = 0.001}; 
          ++d;
          memp += mems[i]**2; 
          } 
        }  
...
async function timerun() { 
  ...
  function prmem() {
    ...
    (memrms > 200 || counter2 > 300) ? document.getElementById('aler2').innerText = ' Be careful your transactions may be delayed! ' :  document.getElementById('aler2').innerHTML = '&nbsp';
    ...
    document.getElementById('unconf').innerHTML = memrms;	
  }
};
